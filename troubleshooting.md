...
- **CLI agent 每回合 exit 1、报 `Invalid MCP configuration: MCP config file not found`**——你在 `AGENT_CLI_CMD` 里写死了 `--mcp-config <路径>`（常见于 Windows 的 `C:\Users\...\...json`），但那个文件 consumer 从没生成（通常因为你没连任何 MCP server）。这会让 Claude 每个**聊天**回合都 exit 1；而后台 proactive 回合不带 `--mcp-config`，照常运行——于是表现为“用户收不到聊天回复，但 AI 还在不停自己发消息”。改法：把写死的 `--mcp-config <路径>` 换成 `{mcp}` 占位（如 `... {mcp} ...`），consumer 会在你有启用的 server 时自动填 `--mcp-config <USER_MCP_FILE>`、没有时留空，永不出错。新版 consumer 也会在该文件不存在时自动丢弃这个参数并告警，让 agent 至少能启动。
