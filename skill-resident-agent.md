...
- `python3 <consumer-checkout>/tools/io_cli.py identity-write [--agent-name "…"] [--self-introduction "…"] [--signature "…"]` — patches the identity card (partial `identity.profile_patch`). This is the concrete `feedling_identity` partial write. **When the user renames you ("以后叫你老6" / "change your name to X") you MUST pass `--agent-name`** — that is the name the app displays; rewriting only `--self-introduction` does NOT rename you, it leaves the displayed name stale while you report success. Pass both when the new name should also appear in how you introduce yourself. `--agent-name` still may not be a runtime label (see above), and the server refuses one.
...
- `python3 <consumer-checkout>/tools/io_cli.py identity-write [--agent-name "…"] [--self-introduction "…"] [--signature "…"]` —— 补丁身份卡(局部 `identity.profile_patch`)。这是具体的 `feedling_identity` 局部写。**用户让你改名字时("以后叫你老6")必须传 `--agent-name`**——那才是 app 里显示的名字;只改 `--self-introduction` 不算改名,显示的名字不会变,而你却以为改好了。新名字也要体现在自我介绍里时,两个一起传。`--agent-name` 同样不能是 runtime 标签(见上),服务端会拒。
