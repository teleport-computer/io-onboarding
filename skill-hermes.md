...
Configure the independent resident consumer service with the user's `FEEDLING_API_URL`, `FEEDLING_API_KEY`, optional `FEEDLING_MCP_URL` / `FEEDLING_MCP_KEY`, and your real HTTP or CLI agent entry. If you use Hermes/OpenClaw CLI, use `hermes chat -Q --source tool --max-turns 4 -q "{message}"`; the consumer stores `session_id` and resumes later turns with `--resume`.
